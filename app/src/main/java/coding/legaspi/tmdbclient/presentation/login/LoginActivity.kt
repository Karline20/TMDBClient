            ...
            if (validate()){
                ...
                try {
                    ...
                    responseLiveData.observe(this, Observer {
                        if (it!=null){
                            Log.d("Login", "Login 0 ${it.body()}")
                            val response = it.body()
                            Log.d("Login", response?.token.toString())
                            Log.d("Login", "Login 1 $it")
                            if (response?.username.equals("admin")){
                                Log.d("Login", "Success "+response?.token.toString())
                                binding.progressBar.visibility = GONE
                                sharePreferences.edit().putString("token", it.body()?.token.toString()).apply()
                                val intent = Intent(this, HomeActivity::class.java)
                                startActivity(intent)
                                finish()
                            }else{
                                binding.progressBar.visibility = GONE
                                Log.d("Login", "Failed "+response?.token.toString())
                            }
                        }else{
                            binding.progressBar.visibility = GONE
